...
const endpoints = [
  {
    ...
    url: resolvePathJoin(
      process.env.NEXT_PUBLIC_MINECRAFT_ENDPOINT ||
        'https://staging-minecraft.innercircle-chainmind.com/',
...
endpoints.forEach((endpoint) => {
  const openapiCommand = `bun run openapi-rq -i ${endpoint.url} -o services/${endpoint.name} ${endpoint.command} -c @hey-api/client-axios   --format biome --lint biome --enums typescript --useDateType  `;
  try {
