...
const PROJECT_ROOT = path.join(__dirname, '..'); // Adjust for the /scripts directory
...
const TARGET_LANGUAGES = (process.env.NEXT_PUBLIC_LOCALES ?? 'en')?.split(',');
    ...
    if (stat.isDirectory()) {
      ...
      if (hashStore[relativePath] !== fileHash) {
        ...
        for (const targetLanguage of TARGET_LANGUAGES) {
          console.log('🚀 ~ translateFiles ~ targetLanguage:', targetLanguage);
...
async function translateStrings(
  ...
  for (let i = 0; i < strings.length; i += batchSize) {
    ...
    try {
      ...
      const cleanedTranslatedLines = translatedLines.map((line: any) => {
