...
interface RowType extends Asset {
  onClick: () => void;
  isOpen?: boolean;
}
...
const AssetsTable = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      field: "actual_price",
      modify: (row: RowType) => (
        <ColoredTypography value={row?.actual_price} customColor="white" />
      ),
    },
    {
    ...
    },
    {
      ...
      modify: (row: RowType) =>
        `$${numeral(row?.total_invested).format("0,0.00")}`,
    },
    {
      title: formatTitle(t("assetsTable.unrealizedpnl")),
      field: "unrealized_pnl",
      modify: (row: RowType) => (
        <ColoredTypography value={row?.unrealized_pnl} />
      ),
    },
    {
