...
import {
  useGetGlobalsDropZone,
  useGetMe,
} from "@/services/minecraft/default/default";
import type { DropZoneCoin } from "@/services/minecraft/mockMinecraftAPI.schemas";
import { toNumber } from "@/utils/toNumber";

const columns = [
  {
    title: "Name",
    modify: (row: DropZoneCoin) => (
      <Stack direction="row" alignItems="center" gap={1.5}>
        <Image
          src={CMSDownloadURL((row?.icon as any)?.url!)}
          width={24}
          height={24}
        />
  ...
  {
    title: "cmrValue",
    modify: (row: DropZoneCoin) => row.cmrValue,
  },
  {
    title: "potentialMultiplicator",
    modify: (row: DropZoneCoin) => row.potentialMultiplicator,
  },
];

const DropZone: FC = () => {
  const { data } = useGetGlobalsDropZone();
  const dropZone = data?.data;
  const [isClient, setIsClient] = useState(false);
  const [finishedCountdown, setFinishedCountdown] = useState(false);
  useEffect(() => {
    if (!dropZone?.timestampUnix) {
      return;
    }

    setFinishedCountdown(toNumber(dropZone.timestampUnix) * 1000 < Date.now());
  }, [dropZone?.timestampUnix]);
  ...
  const filteredCoins =
    dropZone?.coins?.filter((item: any) =>
      item.membership?.some((mem: any) => mem.slug === userPlan),
    ) ?? [];
  return dropZone?.status === "active" && finishedCountdown ? (
    <CustomTable title="Drop Zone" columns={columns} data={filteredCoins} />
  ) : (
    <Stack
      position="relative"
      overflow="hidden"
      ...
      >
        {isClient &&
          (dropZone?.status === "inactive" ? (
            ...
            <FlipClockCountdown
              to={new Date(toNumber(dropZone?.timestampUnix) * 1000)}
