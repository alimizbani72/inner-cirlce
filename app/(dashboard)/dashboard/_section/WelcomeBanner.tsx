...
import { useGetGlobalsVideoGlobal } from '@/services/cms/global-videoglobal/global-videoglobal';
...
const WelcomeBanner: FC = () => {
  ...
  const { data } = useGetGlobalsVideoGlobal({ locale: lang });
  const vid = data?.data.videos?.filter((item: any) => item.videoPage === convertRoute(pathName));
        ...
        {userInfo?.data && (
          <>
            ...
            >
                ...
                <Stack gap={1}>
                  <Box>
                    <Typography variant="h3-semi-bold">
                      {t('welcomeBanner.hi', { name: toTitleCase(userInfo?.data?.full_name!) })}
                    </Typography>
              ...
              <Stack
                p={1}
                pl={getUserPlanType(userInfo?.data) !== 'fisherman_plus' ? 2 : 3}
                pr={getUserPlanType(userInfo?.data) !== 'fisherman_plus' ? undefined : 4}
                gap={'28px'}
