...
const CategoryAutoComplete = ({
  ...
  const { data: categoriesData } = useGetCoinReportCategories();
      ...
      renderValue={(categoryVal) => (
        ...
        >
          <Typography variant="p2-medium">
            {toTitleCase(categoryVal?.label as string)}
          </Typography>
          <Box
            sx={{
              cursor: "pointer",
              path: { stroke: (theme) => theme.palette.grey.light },
            }}
            onClick={() =>
              setCategories((prev) =>
                prev.filter((item) => item.value !== categoryVal.value),
              )
            }
          >
