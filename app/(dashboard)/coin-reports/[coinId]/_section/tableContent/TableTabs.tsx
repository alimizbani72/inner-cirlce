...
const TableTabs = ({ contentData, selectedTab, onSelect }: TableTabsProps) => {
  ...
  const keys = contentData ? (Object.keys(contentData) as SelectedTabKey[]) : [];
  return (
    <Stack
      p={3}
      sx={{ bgcolor: 'dark.2', border: '1px solid', borderRadius: 2, borderColor: 'dark.3' }}
      spacing={3}
    >
      <Typography variant="h4-semi-bold">{t('coinReportSingleView.tableOfContent')}</Typography>
      <Stack spacing={2}>
        {keys.map((key, index) => (
          <Stack key={key} spacing={2} sx={{ cursor: 'pointer' }}>
            <Typography
