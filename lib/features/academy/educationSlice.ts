...
export const educationSlice = createSlice({
  ...
  reducers: {
    ...
    setStatus: (state, action: PayloadAction<EducationState['status']>) => {
  ...
  selectors: {
    ...
    selectVideoByTitle: (state) => (videoTitle: string, moduleTitle: string) => {
      for (const module in state.videos) {
        const video = state.videos[module].find(
          (video) => video.title === videoTitle && moduleTitle === module
        );
        if (video) {
          return video;
        }
...
export const { setCategories, setPlaylists, setVideos, setStatus } = educationSlice.actions;

export const { selectCategories, selectPlaylists, selectVideos, selectVideoByTitle, selectStatus } =
