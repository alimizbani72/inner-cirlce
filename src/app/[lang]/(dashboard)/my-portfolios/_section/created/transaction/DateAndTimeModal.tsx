...
type DateModalProps = {
  open: boolean;
  close: VoidFunction;
  onConfirm?: (dateTime: { date: Date | null; time: Date | null }) => void;
  initialDateTime?: { date: Date | null; time: Date | null };
};

export default function DateAndTimeModal({ open, close, onConfirm, initialDateTime }: DateModalProps) {
  const { t } = useTranslate();
  const [date, setDate] = useState<Date | null>(initialDateTime?.date || null);
  const [time, setTime] = useState<Date | null>(initialDateTime?.time || null);
