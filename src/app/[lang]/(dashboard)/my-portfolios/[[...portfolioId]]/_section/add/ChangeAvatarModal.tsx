...
import { Avatar, Button, DialogActions, DialogContent, DialogTitle, Divider, Stack, Typography } from "@mui/material";
...
const ChangeAvatarModal = ({ open, close, onSave, bgColor, portfolioAvatar }: AddPortfolioProps) => {
  ...
  const displayedBgColors = useMemo(() => (isMobile ? avatarBgColors.slice(0, 6) : avatarBgColors), [isMobile]);

  const displayedemojiList = useMemo(() => (isMobile ? emojiList.slice(0, 12) : emojiList), [isMobile]);
  const handleSaveChanges = () => {
    onSave(localBgColor, localPortfolioAvatar);
    close();
  };
  return (
    <CustomDialog fullWidth maxWidth="sm" onClose={close} aria-labelledby="ChangeAvatar-dialog" open={open}>
        ...
        <Stack spacing={3}>
          <Stack direction={{ xs: "column", md: "row" }} justifyContent={"space-between"}>
            ...
            <Stack
              ...
              spacing={1}
              alignItems={"center"}
              sx={{ alignItems: "center", pl: { xs: 1.5, md: undefined }, pt: { xs: 3, md: undefined } }}
            >
          ...
          <Stack direction={"row"} flexWrap={"wrap"} spacing={4} px={1.5} sx={{ cursor: "pointer" }}>
