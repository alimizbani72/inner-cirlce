...
export const pageSlice = createSlice({
  ...
  initialState,
  reducers: (create) => ({
    setPageInfo: create.reducer((state, action: PayloadAction<{ title: string; hasBackButton?: boolean }>) => {
      state.title = action.payload.title;
      state.hasBackButton = !!action.payload.hasBackButton;
    }),
  }),
  selectors: { pageTitle: (state) => state.title, pageHasBackButton: (state) => state.hasBackButton },
});
