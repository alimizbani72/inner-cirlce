...
export const removeIranCode = (input: string | undefined) =>
  input ? `0${input.slice(3)}` : "";

export const toMinutes = (input: number | undefined) =>
  input ? Math.ceil(input / 60) : 0;
...
export const getProgress = (
  ...
  const adjustedProgress = isReady
    ? numericalProgress
    : Math.round(numericalProgress * 0.9);
...
export const formatCurrencyWithoutDollar = (props: undefined | any) =>
