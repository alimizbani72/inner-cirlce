...
const softVariant: Record<string, ComponentsVariants<Theme>['MuiChip']> = {
  colors: COLORS.map((color) => ({
    ...
    style: ({ theme }) => ({
      ...
      '&:hover': { backgroundColor: varAlpha(theme.vars.palette[color].main, 0.32) },
  ...
  inheritColor: [
    {
      ...
      style: ({ theme }) => ({
        backgroundColor: varAlpha(theme.vars.palette.grey['light'], 0.16),
        '&:hover': { backgroundColor: varAlpha(theme.vars.palette.grey['light'], 0.32) },
      }),
    },
  ],
};
    ...
    outlined: ({ ownerState, theme }) => {
      const styled = {
        defaultColor: {
          ...(!ownerState.disabled &&
            ownerState.color === 'default' && {
              borderColor: varAlpha(theme.vars.palette.grey['light'], 0.32),
            }),
        },
      };
      return { ...styled.defaultColor };
