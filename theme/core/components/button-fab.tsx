...
const outlinedVariant: Record<string, ComponentsVariants<Theme>['MuiFab']> = {
  colors: COLORS.map((color) => ({
    ...
    style: ({ theme }) => ({
      ...
      border: `solid 1px ${varAlpha(theme.vars.palette[color].main, 0.48)}`,
      '&:hover': { backgroundColor: varAlpha(theme.vars.palette[color].main, 0.08) },
    }),
  })),
  base: [
    {
      ...
      style: ({ theme }) => ({
        ...
        border: `solid 1px ${varAlpha(theme.vars.palette.grey['light'], 0.32)}`,
        '&:hover': {
...
const softVariant: Record<string, ComponentsVariants<Theme>['MuiFab']> = {
  colors: COLORS.map((color) => ({
    ...
    style: ({ theme }) => ({
      ...
      backgroundColor: varAlpha(theme.vars.palette[color].main, 0.16),
      '&:hover': {
        boxShadow: 'none',
        backgroundColor: varAlpha(theme.vars.palette[color].main, 0.32),
      },
  ...
  base: [
    {
      ...
      style: ({ theme }) => ({
        ...
        [`&.${fabClasses.colorInherit}`]: {
          ...
          '&:hover': { backgroundColor: varAlpha(theme.vars.palette.grey['light'], 0.24) },
        },
      }),
    },
  ],
